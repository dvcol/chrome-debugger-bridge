...
Run `pnpm verify` for build, unit tests, type checks, lint (including browser import restrictions), Chromium browser tests, extension tests, package boundaries and publint. Performance acceptance is warm single-action p95 at or below 500 ms and compact-snapshot p95 at or below one second on the normal fixture. Record the environment and raw samples; exclude model time and explicit application waits. Token reports must identify their estimator and include catalogue, arguments, responses and workflow costs.
