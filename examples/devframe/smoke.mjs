...
import { createDevframeBridgeClient } from '@dvcol/chrome-debugger-bridge-devframe/client';
import { mountDevframeChromeDebuggerBridge } from '@dvcol/chrome-debugger-bridge-devframe/node';
...
async function runSmoke() {
  ...
  bridge.broker.registerTargetExecutor(target, {
    async execute() {
      return { result: { type: 'string', value: 'Devframe packed example' } };
    },
  });

  try {
    const targets = await client.listTargets();
    if (targets[0]?.id !== target.id) throw new Error('Expected the published target through Devframe birpc.');
    ...
    if (event.value?.method !== 'Runtime.consoleAPICalled') throw new Error('Expected the Devframe event stream.');
  ...
  if (server.listenerCount('upgrade') !== 0) throw new Error('Expected complete Devframe disposal.');
}
