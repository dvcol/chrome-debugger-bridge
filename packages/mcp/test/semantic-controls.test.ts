...
it.each(['load', 'domcontentloaded', 'commit'] as const)('ignores stale history and child-frame events while reload waits for %s', async (waitUntil) => {
  expect.assertions(4);
  const toolName = 'browser.reload';
  let dispatchCount = 0;
  const renewedTarget = { ...target, generation: 2 };
        ...
        [Symbol.asyncIterator]() {
          return {
            next: async () => request.targetGeneration === 1
              && ['Page.navigatedWithinDocument', 'Page.frameNavigated'].includes(request.match.method)
              ? {
                  done: false as const,
                  value: {
                    method: request.match.method,
                    parameters: { frameId: 'child-frame', frame: { id: 'child-frame', parentId: 'frame-1' }, url: 'https://example.test/previous-route' },
                    sequence: 1,
                    subscriptionId: crypto.randomUUID(),
                    targetGeneration: 1,
                    targetId: target.id,
              ...
              : request.targetGeneration === 2 && request.match.method === (waitUntil === 'load' ? 'Page.loadEventFired' : 'Page.domContentEventFired')
                ? {
                    done: false as const,
                    value: {
                      method: request.match.method,
  ...
  const session = createCdbToolSession({ client });
  const targetRef = session.projectTarget(target)?.targetRef;
  const navigate = session.definitions.find(tool => tool.name === toolName);
  if (navigate === undefined || targetRef === undefined) throw new Error('browser.navigate is missing');
  ...
  expect(result.isError).toBeUndefined();
  expect(payload.target.targetRef).toBe('t1');
  expect(waitUntil === 'commit' ? payload.milestone : payload.milestone.method).toBe(waitUntil === 'commit' ? 'authority-renewed' : waitUntil === 'load' ? 'Page.loadEventFired' : 'Page.domContentEventFired');
  expect(dispatchCount).toBe(1);
});

it.each(['browser.navigate', 'browser.reload'])('fails %s when renewed authority is not published', async (toolName) => {
  expect.assertions(2);
  const client = {
    ...
    async cancelCommand() {},
    async executeCommand(command: CdpCommand) {
      return { operationId: command.operationId, value: toolName === 'browser.reload' ? {} : { frameId: 'frame-1', loaderId: 'loader-1' } };
    },
    async listTargets() {
      return [target];
    },
  ...
  const result = await navigate.invoke({
    ...
    ...(toolName === 'browser.navigate' ? { url: 'https://next.example.test/' } : {}),
  });
