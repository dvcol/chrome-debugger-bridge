...
export const browserControlNotificationStyles = `
  ...
  * {
  ...
  article {
    position: relative;
    margin: 8px 0;
    padding: 0 var(--cdb-spacing) 12px;
  ...
  h2 {
    ...
    border-radius: var(--cdb-radius) var(--cdb-radius) 0 0;
    background: var(--cdb-surface-muted);
    font: 700 13px/1.3 var(--cdb-font);
    text-align: left;
  }
  p {
    margin: 0 0 8px;
  ...
  button {
    ...
    border-radius: var(--cdb-control-radius);
    padding: 7px 11px;
    font: 600 12px/1.2 var(--cdb-font);
    background: var(--cdb-surface-muted);
    color: var(--cdb-secondary-text);
    cursor: pointer;
  }
  footer {
    display: flex;
  ...
  code {
    ...
    padding: 3px;
    background: transparent;
    font: 12px/1.4 var(--cdb-monospace-font);
  }
  [data-level=observe] {
    color: var(--cdb-observe);
  }
  [data-level=inspect] {
    color: var(--cdb-inspect);
  }
  [data-level=interact] {
    color: var(--cdb-interact);
  }
  [data-level=debug] {
    color: var(--cdb-debug);
  }
  [data-level=unsafe] {
    color: var(--cdb-unsafe);
    font-weight: 700;
  }
  button:focus-visible {
    outline: 2px solid var(--cdb-primary);
    outline-offset: 2px;
  }
  button:disabled {
    opacity: .6;
...
function applyVariables(style: CSSStyleDeclaration, tokens: Readonly<Record<string, string>>): void {
  for (const [name, value] of Object.entries(tokens)) {
    const property = `--cdb-${name.replaceAll(uppercaseCharacter, character => `-${character.toLowerCase()}`)}`;
    style.setProperty(property, value);
  }
}

/** Updates only theme rules, preserving controls and the later custom stylesheet. */
export function updateNotificationTheme(
  ...
  stylesheet.replaceSync(`
    :host {}
    :host, :host(:where([data-color-mode=light])) { color-scheme: light; }
    :host(:where([data-color-mode=dark])) { color-scheme: dark; }
    @media (prefers-color-scheme: dark) {
      :host(:where([data-color-mode=system])) { color-scheme: dark; }
    }
  `);
  const sharedRule = stylesheet.cssRules[0] as CSSStyleRule;
  const lightRule = stylesheet.cssRules[1] as CSSStyleRule;
  const darkRule = stylesheet.cssRules[2] as CSSStyleRule;
  const systemRule = (stylesheet.cssRules[3] as CSSMediaRule).cssRules[0] as CSSStyleRule;
  applyVariables(sharedRule.style, { ...shared, ...sharedOverrides });
  applyVariables(lightRule.style, { ...light, ...lightOverrides, ...branding });
  applyVariables(darkRule.style, { ...dark, ...darkOverrides, ...branding });
  applyVariables(systemRule.style, { ...dark, ...darkOverrides, ...branding });
}
